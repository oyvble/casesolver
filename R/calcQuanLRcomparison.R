...
#' @param xi Model parameter for Backward stutter. Default is 0 (no stutter). NULL indiciates stutter model.
...
#    if(length(x)==1) x <- rep(x,2)
 ...
 #NOTICE: These values can be stored for later for deconvolution
  ...
  for(rr in whatR ) { #for each references
   ...
   #Calc Lik(E|Hp)
   if(nC>2) fitHp <- euroformix::contLikMLEpara(nC,sample,data2$popFreq,data2$refData,condOrder=1,xi=xi,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
   if(nC<=2) fitHp <- euroformix::contLikMLE(nC,sample,data2$popFreq,data2$refData,condOrder=1,xi=xi,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
